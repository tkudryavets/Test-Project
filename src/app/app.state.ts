...
export class AppState {
    ...
    getDataFromState(ctx: StateContext<PlanStateModel>) {
        return this.plansService.getPlans().pipe(tap(returnData => {
    ...
    addDataToState(ctx: StateContext<PlanStateModel>, { payload }: AddPlan) {
        return this.plansService.addPlan(payload).pipe(tap(returnData => {
            const state=ctx.getState();
            ctx.patchState({
    ...
    @Action(UpdatePlan)
    updateDataOfState(ctx: StateContext<PlanStateModel>, { payload }: UpdatePlan) {
        return this.plansService.updatePlan(payload).pipe(tap(returnData => {
            const state=ctx.getState();

            const userList = [...state.Plans];
