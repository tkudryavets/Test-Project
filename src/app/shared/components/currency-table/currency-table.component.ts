...
export class CurrencyTableComponent implements OnInit{
  ...
  private createImageFromBlob(image: Blob) {
    ...
    reader.addEventListener(
      ...
      () => {
        ...
        this.isLoaded = true;
        (this.cursor as HTMLDivElement).style.backgroundImage = `url(${this.flag})`
      },
      false
    );
  ...
  public cursorMove(){
    const positionElement = (e: any)=> {
      ...
      (this.cursor as HTMLDivElement).style.transform = `translate3d(calc(${mouseX}px + 2rem), calc(${mouseY}px + 2rem), 0)`;     
    }
